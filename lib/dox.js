...
function getTypeImportData(tImport, tag) {
  ...
  if (!exists) {
    ...
    throw new Error(`Unable to find ${tImport.importPath} in ${tag.tagValue}. Please double check this import` )
  }
...
exports.parseTagTypes = function(str, tag, allText, ref) {
  ...
  if (tag) {
    ...
    tag.optional = (tag.nameRaw && tag.nameRaw.slice(0,1) === '[') || result.optional || optional
    // console.log('tag.optional', tag.optional)
    // console.log('result.optional', result.optional)
    // tag.type = (result && result.toHtml) ? result.toHtml() : result;
