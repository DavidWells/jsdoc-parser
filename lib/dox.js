...
const optsParse = require('oparser').parse
...
// const PARAM_REGEX = /\[([$a-zA-Z0-9._-]*)=?([$a-zA-Z0-9_.='" \t\[\]\{\}/\\]*)\]/
const PARAM_REGEX = /^\[([$a-zA-Z0-9._-]*)\s?=?\s?(.*)\]/
/**
 * Expose api.
 */
...
const LINTER_PREFIXES = ['jslint', 'jshint', 'eshint', 'eslint']
...
function parseComments(js, options) {
  ...
  var comments = [],
    ...
    withinString = false,
    code,
    linterPrefixes = options.skipPrefixes || LINTER_PREFIXES,
  ...
  for (var i = 0, len = js.length; i < len; ++i) {
    ...
    // end comment
      ...
      // console.log('xyz comment', comment)
  ...
  if (options.excludeIgnored) {
    return excludeIgnoredComments(allComments)
  }

  /* Validation Rules */
  allComments.forEach((comment) => {
    comment.validationErrors = []
    // Check for duplicate params and keep track of the duplicate names and the count
    const paramNames = comment.tags.filter((tag) => tag.tagType === 'param').map((tag) => tag.name)
    const uniqueParamNames = [...new Set(paramNames)]
    if (paramNames.length !== uniqueParamNames.length) {
      const duplicateNames = paramNames.filter((name, index, self) =>
        self.indexOf(name) !== index
      )
      duplicateNames.forEach((name) => {
        const count = paramNames.filter((n) => n === name).length
        comment.validationErrors.push({ 
          type: 'DUPLICATE_PARAM', 
          message: `Duplicate parameter name: ${name} (used ${count} times)`,
          // todo Line context
        })
      })
    }
...
function excludeIgnoredComments(allComments) {
  return (allComments || []).filter((comment) => {
    return !comment.isIgnored
  })
}

exports.excludeIgnoredComments = excludeIgnoredComments
...
exports.parseComment = function(str, options, allText, ref) {
  ...
  // parse comment body
  let tagText = tags[0]
  const startsWithExclamation = tagText[0] === '!'
  // console.log('startsWithExclamation', startsWithExclamation)
  if (startsWithExclamation) {
    /* remove leading ignore ! for output */
  ...
  comment.tags = []
  comment.isIgnored = startsWithExclamation

  // parse tags
  if (tags.length) {
    comment.tags = tags.slice(1).map((t) => {
      return exports.parseTag(t, allText, comment.description, ref, options)
    })

    const hasIgnoreTag = comment.tags.some((tag) => tag.tagType === 'ignore')
    if (hasIgnoreTag) {
      comment.isIgnored = true
    }

    const typeDefs = comment.tags.filter((tag) => tag.tagType === 'typedef')
    
    const hasSingleTypeDef = typeDefs.length === 1
    if (hasSingleTypeDef && typeDefs[0].name) {
      comment.type = typeDefs[0].name
    }
    ...
    if (!description.text || !description.text.trim()) {
      const descriptionTag = comment.tags.find(function(tag) {
        return tag.tagType === 'description' || tag.tagType === 'desc'
      })
      // console.log('descriptionTag', descriptionTag)
      /* If has description tag, use it */
      if (descriptionTag) {
         description.text = descriptionTag.tagValue
         description.summary = descriptionTag.summary
         description.body = descriptionTag.body
      } else if (hasSingleTypeDef && typeDefs[0].description) {
        /* Else Use typedef description if it exists */
        description.text = typeDefs[0].description
        description.summary = typeDefs[0].description
        description.body = typeDefs[0].description
      }
    }
  }

  if (comment && typeof comment.type === 'undefined') {
    delete comment.type
  }
 ...
 * @api public
 */
exports.parseTag = function(str, allText, desc, ref, options) {
  var tag = {}
    lines = str.split('\n'),
  ...
  switch (type) {
    ...
    case 'param':
      ...
      if (paramsMatch) {
        ...
        // console.log('paramsMatch[2]', `"${paramsMatch[2]}"`)
        /* Set default values */
        if (paramsMatch[2]) {
          // console.log('paramsMatch[2]', paramsMatch[2])
          if (paramsMatch[2].indexOf('{') > -1 || paramsMatch[2].indexOf('[') > -1) {
            const val = optsParse(`defaultVal=${paramsMatch[2]}`)
            tag.defaultValue = val.defaultVal
          } else if (paramsMatch[2] === 'false') {
            tag.defaultValue = false
          } else if (paramsMatch[2] === 'true') {
            tag.defaultValue = true
          } else {
      ...
      tag.name = name
      tag.nameRaw = nameRaw
      tag.description = (partJoin || '').replace(/^\s*-\s*/, '').trim()
      // tag.descriptionRaw = partJoin
      ...
      exports.parseTagTypes(typeString, tag, allText, ref, options);
      break;
    case 'define':
    case 'return':
    case 'returns':
    ...
    case 'type':
      ...
      let typeDefDesc = ''
      if (type === 'typedef' && lines[0]) {
        const stringVal = (parts[0].indexOf('{') === -1) ? parts[0] : parts[1]
        tag.name = stringVal
        tag.nameRaw = stringVal
        if (parts.length > 1) {
          // If has dash split at dash
          if (lines[0].indexOf('-') > -1) {
            const descr = lines[0].split('-')
            // console.log('descr', descr)
            if (descr.length > 1) {
              typeDefDesc = descr[1].trim()
            }
          } else {
            // Split at typeName
            const descr = lines[0].split(stringVal)
            if (descr.length > 1) {
              typeDefDesc = descr[1].trim()
            }
          }
        ...
        if (typeDefDesc) {
          tag.description = typeDefDesc
        } else if (desc) {
          tag.description = desc.summary || desc.text || ''
        }
      }
      exports.parseTagTypes(parts.shift(), tag, allText, ref, options)
      break;
    case 'lends':
    case 'memberOf':
    ...
    case 'throws':
      var typeString = matchTypeStr.test(parts[0]) ? parts.shift() : "";
      tag.types = exports.parseTagTypes(typeString, null, allText, ref, options);
      tag.description = parts.join(' ');
      break;
    case 'description':
    case 'desc':
...
function getTypeImportData(tImport, tag, options) {
  const paths = getPossiblePaths(tImport.importPath, options.cwd)
  //*
 ...
 * @api public
 */
exports.parseTagTypes = function(str, tag, allText, ref, options) {
  if (!str) {
    if (tag) {
  ...
  let jsDocAst = null
  try {
    jsDocAst = parse(strToParse)
  } catch (e) {
    /**  //@typedef {(x?: string, y?: number) => string} LogStackBoxesAPI is broken */
    // Need to replace the ? with | undefined
    // x: string | undefined, y: number | undefined
    function convertOptionalParams(typeString) {
      return typeString.replace(/(\w+)\?\s*:\s*(\w+)/g, '$1: $2 | undefined')
    }
    const fixedStr = convertOptionalParams(strToParse)

    try {
      jsDocAst = parse(fixedStr)
    } catch (e) {
      console.log('JS DOC AST ERROR')
  ...
  if (tag) {
    ...
    if (isImportedType) {
      ...
      console.log('typeImports', typeImports)
      const [ typeData ] = getTypeImportData(typeImports, tag, options)
      if (typeData) {
