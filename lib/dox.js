...
function getPossiblePaths(importPath, cwd) {
  const basePath = path.resolve(cwd || process.cwd(), importPath)
  return [
    basePath,
    `${basePath}.ts`,
    `${basePath}.js`,
  ...
  if (!raw) {
    ...
    comment.tags.forEach(function (tag) {
      if (tag.description) {
        ...
        tag.description = markdown.render(desc).trim();
      } else {
...
function getTypeImportData(tImport) {
  ...
  if (exists) {
    ...
    if (importContents) {
      ...
      extraTypes = typeConverter(importContents)
...
exports.parseTagTypes = function(str, tag, fullText, ref) {
          ...
          if (!isBrowser) {
            ...
            if (typeImports && typeImports.length) {
              ...
              const importFiles = typeImports.map((tImport) => {
                ...
                const importInfo = getTypeImportData(tImport)
  ...
  if (tag) {
    ...
    if (isImportedType) {
      ...
      tag.importedType = getTypeImportData(typeImports)
    }
